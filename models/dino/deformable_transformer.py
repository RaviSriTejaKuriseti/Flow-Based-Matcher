...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        self.apply_DSQ = apply_DSQ
        self.DQS_nms_conf = DQS_nms_conf
        assert query_dim == 4
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        if self.two_stage_type =='standard':
            ...
            #Ajay version on Dense Sparse Queries
            if self.apply_DSQ:
                final_output_indices = []
                prob_values_logits = enc_outputs_class_unselected.max(-1)[0]
                bbox_values = box_ops.box_cxcywh_to_xyxy(enc_outputs_coord_unselected.detach().sigmoid())

                #have to implement a for loop here to apply nms over each of them so that 
                for b_i in range(bbox_values.shape[0]):
                    prob_values = (prob_values_logits[b_i]-torch.min(prob_values_logits[b_i]))/(torch.max(prob_values_logits[b_i])-torch.min(prob_values_logits[b_i]))
                    post_nms_indices = torchvision.ops.nms(bbox_values[b_i],prob_values,self.DQS_nms_conf)        
                    if len(post_nms_indices) < topk:
                        post_nms_indices = torchvision.ops.nms(bbox_values[b_i],prob_values,self.DQS_nms_conf+0.05) 
                    if len(post_nms_indices) < topk:
                        post_nms_indices = torchvision.ops.nms(bbox_values[b_i],prob_values,self.DQS_nms_conf+0.1)
                    if len(post_nms_indices) < topk:
                        post_nms_indices = torchvision.ops.nms(bbox_values[b_i],prob_values,self.DQS_nms_conf+0.15)
                    if len(post_nms_indices) < topk:
                        post_nms_indices = torchvision.ops.nms(bbox_values[b_i],prob_values,self.DQS_nms_conf+0.2)       
                    
                    final_output_indices.append(post_nms_indices[:topk])

                    if len(post_nms_indices) < topk:
                        print("Hmmm there is a problem")


            if self.apply_DSQ:
                topk_proposals = torch.stack(final_output_indices)
            else:
            ...
            # softmax_prob = softmax_func(enc_outputs_class_unselected)
            # sorted_proposals = torch.topk(enc_outputs_class_unselected.max(-1)[0],enc_outputs_class_unselected.shape[1], dim=1)[1]
            # logit_values = enc_outputs_class_unselected[0].max(-1)[0][sorted_proposals]
            # prob_values = softmax_prob[0].max(-1)[0][sorted_proposals]
            # persons_prob_values = softmax_prob[0][:,1][sorted_proposals]
            # sorted_refpoint_embed_undetach = torch.gather(enc_outputs_coord_unselected, 1, sorted_proposals.unsqueeze(-1).repeat(1, 1, 4)) # unsigmoid
            # sorted_refpoint_embed_ = box_ops.box_cxcywh_to_xyxy(sorted_refpoint_embed_undetach.detach().sigmoid())
            # sorted_refpoint_embed_numpy = sorted_refpoint_embed_.cpu().detach().numpy()
            # df_embed = pd.DataFrame(sorted_refpoint_embed_numpy[0], columns = ['x1','y1','x2','y2'])
            # df_embed['prob'] = prob_values[0].cpu().detach().numpy()
            # df_embed['person_prob'] = persons_prob_values[0].cpu().detach().numpy()
            # df_embed['logit'] = logit_values[0].cpu().detach().numpy()
            # df_embed.to_csv("/home/ajay_sh/scratch/pedestrian_detection/DETR-Based/align_DINO/DINO/analysis_results/intermediate_results/all_rois.csv",index=False)
...
def build_deformable_transformer(args):
    ...
    return DeformableTransformer(
        ...
        use_detached_boxes_dec_out=use_detached_boxes_dec_out,
        apply_DSQ=args.apply_DSQ,
        DQS_nms_conf = args.DQS_nms_conf
    )
