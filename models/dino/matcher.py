...
from .many_to_one_dummy_gt import match_with_dummy_gt

class MinCostFlowMatcher(nn.Module):
    """This class computes an assignment between the targets and the predictions of the network
    For efficiency reasons, the targets don't include the no_object. Because of this, in general,
    there are more predictions than targets. In this case, we do a n-to-1 matching of the best predictions,
    while the others are un-matched (and thus treated as non-objects).
    """

    def __init__(self, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1, focal_alpha = 0.25, repetition = 6, dummy_weight=1.8):
        """Creates the matcher
        Params:
    ...
    def forward(self, outputs, targets):
        ...
        indices = [match_with_dummy_gt(c[i],self.repetition,self.dummy_weight*10**4) for i, c in enumerate(C.split(sizes, -1))]
