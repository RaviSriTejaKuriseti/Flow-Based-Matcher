...
def IA_BCE_loss(src_logits,pos_idx_c, src_boxes, target_boxes, indices, avg_factor, alpha,gamma):
    prob = src_logits.sigmoid()
    #init positive weights and negative weights
    pos_weights = torch.zeros_like(src_logits)
    neg_weights =  prob ** gamma
    #ious_scores between matched boxes and GT boxes
    iou_scores = torch.diag(box_ops.box_iou(box_ops.box_cxcywh_to_xyxy(src_boxes), box_ops.box_cxcywh_to_xyxy( target_boxes))[0])
